...
if ~exist('numDigits','var')
    numDigits = 1;     %number of digits when adding numbers to a filename
end
numDigits = num2str(numDigits);

if ~exist('startCount','var')
    startCount = 1;     %first value for counter. This determines the first filename during enumeration.
end
